...
- **画像読み込みヒント（2026-09-12、`inc/performance.php`）**: コアは `template` コンテキストの画像に lazy/fetchpriority を付けない（front-page の内容がテンプレートにあると全画像 eager になる）ため、`wp_get_loading_optimization_attributes` フィルターで補完。先頭3枚（`wp_omit_loading_attr_threshold()`）まで eager・最初の大きな画像（≥50000px²）に high、以降の寸法付き画像は lazy。寸法なし・属性済みは不干渉。theme_mod `shitate_image_loading`（カスタマイザー「パフォーマンス」）と `shitate_optimize_image_loading` フィルターで無効化＝WP Rocket 等との干渉回避
